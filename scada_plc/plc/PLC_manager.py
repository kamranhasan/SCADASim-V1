...
if __name__ == "__main__":
    ...
    keep = True
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hkvf:n:", ["keep", "help", "version", "file="])
    ...
    try:
        if config:
            ...
            while True:
                try:
                    if requests.get("http://localhost:5001/api/").status_code == 200:
                        config = requests.get("http://localhost:5001/api/controller/config").json()
                        break
