...
modbus_ip=$(vmtoolsd --cmd 'info-get guestinfo.modbus.address')
modbus_mask=$(vmtoolsd --cmd 'info-get guestinfo.modbus.netmask')
host=$(vmtoolsd --cmd 'info-get guestinfo.hostname')
port=$(vmtoolsd --cmd 'info-get guestinfo.scada.port')

echo -e '' | tee -a /etc/sysconfig/network-scripts/ifcfg-ens32
echo -e '' | tee -a /etc/sysconfig/network-scripts/ifcfg-ens33
...
echo -e 'DEVICE=ens33' | tee -a /etc/sysconfig/network-scripts/ifcfg-ens33
echo -e 'ONBOOT=yes' | tee -a /etc/sysconfig/network-scripts/ifcfg-ens33
echo -e 'IPADDR='$modbus_ip | tee -a /etc/sysconfig/network-scripts/ifcfg-ens33
echo -e 'NETMASK='$modbus_mask | tee -a /etc/sysconfig/network-scripts/ifcfg-ens33
...
# mv /var/www/HMI- /var/www/HMI-$host
...
iptables -I INPUT 1 -p tcp -m tcp --dport 5020 -j ACCEPT
iptables -I INPUT 1 -p tcp -m tcp --dport 5432 -j ACCEPT

/bin/python3 /home/hp/Desktop/SCADASim-38d3abce7cc33026ead9a24372634987d1a6aebd/scada_historian/historian/dataserver.py  -p 5001 -u kamran -d kamran -w kamran
