...
class PLCThread(threading.Thread):
    ...
    # Returns: Initialized PLC Thread object
    def plc_server(self): 
        StartTcpServer(context, identity=identity, address=self.address)
    ...
    def historian_connection_check(self):
        ...
        while True:
            try:
                if requests.get("http://127.0.0.1:5001/api").status_code == 200:
                    print("HERE")
                    self.historian_handler("localhost:5001")
                    return
