...
class DatabaseDriver:
    ...
    def get_actuator_value(self, aid):
        self.curr.execute("SELECT * from 1 ORDER BY event_time DESC LIMIT 1" )
        return self.curr.fetchone()
    ...
    def set_actuator_value(self, aid, new_value, uid):
        print("UID: %s UID Type: %s" % (uid, type(uid)))
    ...
    def get_sensor_data(self, sid, count=True):
        ...
        for i in range(0,500):
            if i < len(b):
                coor.append({"value": b[i]["value"], "event_time": now.__str__()})
            else:
                coor.append({"value": b[len(b) - 1]["value"], "event_time": now.__str__()})
    ...
    def build_config_file(self, sub_system=None):
        ...
        for i in device_list:
            if not sub_system and "his" in i['id']:
                return self.get_device_map(i, device_list, plc_list)
            elif sub_system and i["id"] == sub_system:
                return self.get_device_map(i, device_list, plc_list)
        else:
    ...
    def get_root_id(self):
        ...
        if 'his' in str(device_list):
            ...
            slave_list = self.get_device_list()
        for i in device_list:
            if i not in str(slave_list):
                return i
